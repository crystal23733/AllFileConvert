...
describe("ResultPreview", () => {
  ...
  it("이미지 타입이면 <img> 렌더링", () => {
    render(<ResultPreview url={url} type="image" filename={fileName} />);
  ...
  it("비디오 타입이면 <video> 렌더링", () => {
    render(<ResultPreview url={url} type="video" filename={fileName} />);
  ...
  it("PDF 타입이면 <iframe> 렌더링", () => {
    render(<ResultPreview url={url} type="pdf" filename={fileName} />);
  ...
  it("미지원 타입이면 안내 메시지 렌더링", () => {
    render(<ResultPreview url={url} type="unknown" filename={fileName} />);
    expect(screen.getByText("미리보기를 지원하지 않는 파일 유형입니다.")).toBeInTheDocument();
  });

  it("url이 없으면 아무것도 렌더링하지 않는다", () => {
    const { container } = render(<ResultPreview url="" type="image" />);
